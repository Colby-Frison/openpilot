...
def test_fill_model_msg_populates_core_contract_fields():
  ...
  assert base_msg.drivingModelData.action == "none"
  assert model_v2.action == "none"
  assert model_v2.confidence in (ConfidenceClass.green, ConfidenceClass.yellow, ConfidenceClass.red)



def test_fill_model_msg_clamps_negative_frame_age_to_zero():
  base_msg = DummyBuilder()
  extended_msg = DummyBuilder()
  outs = _build_model_output_data()
  state = PublishState()
  ...
  fill_model_msg(
    ...
    frame_drop=0.12,
    timestamp_eof=123456,
    model_execution_time=0.045,
    valid=True,
  )

  model_v2 = extended_msg.modelV2
  driving = base_msg.drivingModelData

  assert model_v2.frameAge == 0
  assert model_v2.frameDropPerc == pytest.approx(12.0, abs=1e-6)
  assert driving.frameDropPerc == pytest.approx(12.0, abs=1e-6)
  assert model_v2.modelExecutionTime == pytest.approx(0.045, abs=1e-6)
  assert driving.modelExecutionTime == pytest.approx(0.045, abs=1e-6)
